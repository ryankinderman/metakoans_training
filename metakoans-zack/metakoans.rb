...
#
  def koan_6
    ...
    assert{ first.a? }
    assert{ first.b? }
    assert{ first.a = nil; first.a == nil }
    assert{ first.a == nil }
    puts "think about how the scoping of variables"
    assert{ first.b != nil }
    assert{ first.a == nil }
    assert{ first.b = nil; first.b == nil }
    assert{ first.a == nil }
    first.a = nil
    puts "first.a = '#{first.a}'"
    puts "first.a? = '#{first.a?}'"
    assert{ not first.a? }
    assert{ not first.b? }
...
abort "usage: #{ $0 } knowledge_file or have knowledge.rb" unless knowledge 
